...
这样可以减少本地网络直接看到明文 DNS 查询的机会，但并不隐藏所有目标信息：目标 IP、流量特征，以及未使用 ECH 时可能暴露的 TLS SNI 仍可能提供域名线索。所以在代理链路中，不只是网页请求需要处理，DNS、路由和 TLS 元数据也需要一起考虑。
...
如果 DNS 和目标连接都正确地经由隧道处理，中间网络通常能看到你连接的代理节点及流量元数据，但不能直接读取隧道内的应用数据。代理服务端则知道目标地址；如果目标本身使用 HTTPS，代理层解开隧道后看到的仍可能是另一层 TLS 密文，是否能看到正文取决于 TLS 在哪里终止。
...
VPS，全称是 Virtual Private Server，也就是虚拟专用服务器。在这类代理链路中，VPS 运行远端代理服务。它先处理代理协议和隧道加密，再根据目标地址建立出站连接。这里的“解密”通常指代理隧道层；若用户访问 HTTPS 网站且未做 TLS 中间人，网站 TLS 仍在用户应用与目标网站之间端到端存在，VPS 不能直接读取网页正文。这个过程可以理解为：
